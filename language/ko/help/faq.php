...
$lang = array_merge($lang, array(
	...
	'HELP_FAQ_FORMATTING_BBOCDE_ANSWER'	=> 'BBCode는 HTML의 특별한 구현으로, 게시물의 특정 개체에 대한 뛰어난 서식 제어를 제공합니다. BBCode의 사용은 관리자에 의해 허용되지만 게시 양식에서 게시물별로 비활성화할 수도 있습니다. BBCode 자체는 HTML과 스타일이 비슷하지만 태그는 &lt; 및 &gt;. BBCode에 대한 자세한 내용은 게시 페이지에서 접근할 수 있는 가이드를 참조하십시오.',
	...
	'HELP_FAQ_FORMATTING_IMAGES_ANSWER'	=> '예, 게시물에 이미지를 표시할 수 있습니다. 관리자가 첨부파일을 허용한 경우 포럼에 이미지를 업로드할 수 있습니다. 그렇지 않으면 공개적으로 접근할 수 있는 웹 서버에 저장된 이미지에 링크해야 합니다. http://www.example.com/my-picture.gif. 자신의 PC에 저장된 사진(공개적으로 접근할 수 있는 서버가 아닌 경우)이나 인증 메커니즘 뒤에 저장된 이미지에 연결할 수 없습니다. hotmail 또는 yahoo 편지함, 암호로 보호된 사이트 등. 이미지를 표시하려면 BBCode [img] 태그를 사용하십시오.',
	...
	'HELP_FAQ_FRIENDS_BASIC_ANSWER'	=> '목록을 사용하여 보드의 다른 구성원을 구성할 수 있습니다. 친구 목록에 추가된 구성원은 온라인 상태를 확인하고 비공개 메시지를 보낼 수 있는 빠른 접근을 위해 사용자 제어판에 나열됩니다. 템플릿 지원에 따라 이러한 사용자의 게시물도 강조 표시될 수 있습니다. 적 목록에 사용자를 추가하면 해당 사용자가 작성한 모든 게시물은 기본적으로 숨겨집니다.',
	...
	'HELP_FAQ_LOGIN_REGISTER_ANSWER'	=> '구지 가입을 하실 필요는 없습니다. 게시물를 게시하기 위해 등록해야 하는지 여부는 포럼 운영자가 결정 합니다. 그리고 수많은 스팸 해킹시도 등을 어느정도 거르기 위해서도 있습니다. 다만 가입하면 아바타 이미지, 개인 메시지, 회원 끼리의 이메일 보내기, 사용자 그룹 구독 등과 같이 게스트 사용자가 사용할 수 없는 추가 기능에 접근할 수 있습니다. 등록하는 데 몇 분 밖에 걸리지 않으므로 등록하는 것을 추천 드립니다.',
	...
	'HELP_FAQ_POSTING_FORUM_RESTRICTED_QUESTION'	=> '포럼에 접근할 수 없는 이유는 무엇입니까?',
	...
	'HELP_FAQ_SEARCH_FORUM_ANSWER'	=> '색인, 포럼 또는 주제 페이지에 있는 검색 상자에 검색어를 입력하십시오. 포럼의 모든 페이지에서 사용할 수 있는 "고급 검색" 링크를 클릭하면 고급 검색에 접근할 수 있습니다. 검색에 액세스하는 방법은 사용된 스타일에 따라 다를 수 있습니다.',
